...
# read all cards
@route.get("", response_model=List[schemas.ReadCards], tags=["Card"])
async def get_cards(skip: int = 0, limit: int = 100) -> Any:
    return await crud.cards.get_all(skip=skip, limit=limit)


# read card by cardID
@route.get("/{id}", response_model=schemas.Card, tags=["Card"])
...
# create card
@route.post("", response_model=schemas.Card, tags=["Card"])
