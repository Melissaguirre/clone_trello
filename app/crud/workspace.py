...
class CRUDWorkspace(CRUDBase[Workspaces, WorkspaceCreate, WorkspaceUpdate]):
    ...
    async def create(self, *, obj_in : WorkspaceCreate) -> Workspaces:
        db_obj = await Workspaces.create(workspaceID = obj_in.workspaceID,
        workspace_name = obj_in.workspace_name, background_imgURL = obj_in.background_imgURL, starred = obj_in.starred, board_type = obj_in.board_type )
        return db_obj
